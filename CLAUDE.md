...
| `contracts/interfaces.md` | 介面契約（封包/序列/命令；非 web 專案的介面真相） | 定義/變更介面時 |
| `contracts/data-schema.sql` | DB 結構契約（**適用才用**，否則 N/A） | 資料模型定案／變更 |
| `contracts/api.openapi.yaml` | API 契約（**適用才用**，否則 N/A） | 介面定案／變更 |
...
- **介面真相看型態**：非 web/DB 專案（嵌入式/協定/CLI）以 `contracts/interfaces.md`（封包/序列/命令）為介面契約；`data-schema.sql`、`api.openapi.yaml` **適用才用**，否則標 `不適用 (N/A)`。
...
- **安全態 / 失效保護**：偵測到**致動器/繼電器/馬達/安全相關輸出**時，強制要求 ①一則**明確的安全態決策**（哪個物理狀態＝安全，依危害模型）②**fail-safe 設計**（安全態＝失電/無驅動時被動落到的狀態，不靠 MCU 持續輸出維持）③`charter/risk-register.md` 記一筆 ④`acceptance/` 一項「各故障情境皆回安全態」的驗證。
...
- **變更準則**：**決策反轉**（如換編譯器/建置方式）→ 新增一則「取代 ADR-XXXX」，舊的標已棄用、內容保留；**只是澄清/把擱置點定下來**（決策不變）→ 原地加「修訂註記」。兩者都走 `changes/change-log.md`。
...
### 技術棧可行性查證（技術棧 ADR 定案前必做）
定案任何選定「裝置/SDK/框架/協定/工具鏈」的 ADR 前，AI 須查證並回報：
- 該 SDK/工具是否支援「**確切裝置型號與矽晶版本**」（非僅晶片家族）？來源？
- 選定**協定/頻段/介面**是否真的被此方案支援（避免選到相近但錯誤的方案）？
- 工具鏈的**確切版本是否存在且可下載**（含 legacy/封存）？官方釘定的編譯器是哪套？
- 實作所需的**具體檔案/標頭/patch/API** 是否存在且可取得（不只「功能有支援」，而是那功能所需的檔真的找得到）？
- 風險最高的介面（無線互通、匯流排協定…）能否用**廠商/獨立工具先對真實對端實測**（寫任何程式碼前先證明可行）？
- 上述每點附**可追溯來源**（官網/release notes）。查不到或有疑義 → 標 ⚠️，不得逕自定案。
> 動機：技術棧一旦定案即連動程式碼生成與大量下游；選錯的代價遠高於定案前多花幾分鐘查證。
...
### 可攜邏輯優先（韌體/嵌入式建議樣式）
韌體大量邏輯（協定編解碼、狀態機、資料格式、門檻計算）其實**與晶片無關**。建議切成：
- **可攜核心**（純語言、無平台相依）→ 可用 **host 編譯器直接編譯 + 單元測試**（`host-selftest`），**沒裝目標 SDK 也能驗證契約**（封包佈局/校驗/狀態轉移…）。
- **HAL 介面 + 平台樁**（GPIO/UART/RF/WDT 等 SDK 相依）→ 未就緒時以 `TODO` 樁存在，待環境就緒再補。
> 因此**優先生成可攜邏輯並以 host 測試驗證契約**，平台相依層待環境就緒（呼應「環境就緒門檻」）。
> `acceptance/run-tests.sh` 可加：偵測到 host 編譯器 → 編譯並跑純邏輯 host 測試（讓 pre-commit 真正有測試可擋）。
...
- **請使用者裝東西前，先驗證來源真的含該依賴**：別憑「通常在某套件裡」的假設連續叫使用者裝大型套件；先查證（官方文件/檔案清單/搜尋已裝）確認再請安裝，無法先驗證就明說並提最低成本查證，落空即止損、不連續猜，已裝優先重用。詳見 `ops/environment.md` 安裝政策。
