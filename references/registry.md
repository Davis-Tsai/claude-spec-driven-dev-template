...
- **含角括號 `<...>` 的行＝尚未設定的佔位符**；填了具體路徑（無 `<>`）就是已設定，Claude 會逐行判斷、對具體路徑直接讀取。
- 行尾小括號 `（…）` 是**給你自己看的註解**，可留、可改、可刪，不影響 Claude 讀取。
- **完整路徑 ＝ 這裡的 Base ＋ 下表「位置」欄**。換機器 / 搬專案 / 換 Drive → 只改這裡對應那一行，整表不用動。

**填寫區**（把下面改成你的實際路徑；用不到的行就刪掉。**不想手動填？把資料夾的 `G:\` 路徑貼給 Claude，它會幫你填進來**）：
...
> ⚠️ 範例可刪：下方為格式示範。**Claude 首次為真實專案登錄時，會清掉這些範例列，以及上方未使用的 BASE 佔位行（含 `<...>`）。**
