...
- **參考 Base 不限 G drive**：本機**相鄰專案 repo**（如對接的對端裝置規格）也可登記為 Base（如 `BASE-C`），當跨專案對接的真相來源。

## 給 Claude 讀取的方式
- **逐行判斷 Base（重要）**：Base 區**每一行獨立判斷** —— 含角括號佔位符（如 `<你的專案>`）＝**未設定**，略過；具體路徑（無角括號）＝**已設定**，可直接讀。
  **只要有任一 Base 是具體路徑，就直接去讀它**；不可因其他 Base 行仍是佔位符、或範例 REF 列還在，就判定整個 references 尚未設定。
...
- **Base（路徑）→ 使用者設定**：Claude 無法得知你的 Drive 路徑/磁碟代號，這行要你填（或 Claude 問你一次）。
  **也可直接把資料夾的 `G:\` 路徑貼給 Claude，由 Claude 代填/更新 Base，不必手動編輯。**
...
- **首次為真實專案登錄時**，Claude 主動清掉**未使用的 BASE 佔位行**（含 `<...>` 者）與**範例 REF 列**，讓表只剩真實內容。
